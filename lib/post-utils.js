...
module.exports = {
  deletePost(absFilename) {
    const [, slug] = absFilename.match(/\d{4}-\d{2}-\d{2}-(.*?)\.md$/);
  ...
  parsePost(absFilename) {
    const [, date, slug] = absFilename.match(/(\d{4}-\d{2}-\d{2})-(.*?)\.md$/);

    let mtime;
    let cachedPost;

    return fsp.stat(absFilename).then(stats => {
      ...
      return fsp.readFile(absFilename, 'utf8');
    }).then(data => {
      if (!cachedPost) {
